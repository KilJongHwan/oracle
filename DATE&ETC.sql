-- 날짜 데이커를 다루는 날짜 함수
-- SYSDATE : 운영체제의 현재 날짜와 시간의 정보를 가져옴
...
SELECT SYSDATE,
    ADD_MONTHS(SYSDATE, 3)  -- 두번째 인자는 달 수를 의미(3개월 이후)
FROM DUAL;

-- EMP Table에서 입사 10주년이 되는 사원들 데이터 출력
SELECT EMPNO, ENAME, HIREDATE,
    ADD_MONTHS(HIREDATE, 120) AS Anniversary_10th
FROM EMP;

-- SYSDATE와 ADD_MONTHS 함수를 사용하여 현재 날짜와 6개월 후 날짜가 출력
SELECT SYSDATE AS 현재날짜,
    ADD_MONTHS(SYSDATE, 6) AS SIXMONTHAGO
FROM DUAL;
...
SELECT EMPNO, ENAME ,HIREDATE, SYSDATE,
    ...
    ROUND(MONTHS_BETWEEN(SYSDATE, HIREDATE)) AS MONTHS_4
FROM EMP;

-- 날짜 정보 추출 함수
-- EXTRAXCT 함수는 날짜 유형의 데이터로쿠터 날짜 정보를 분리하여 새로운 칼럼의 형태로 출력
...
-- 오늘 날짜에 대한 정보 조회
SELECT SYSDATE FROM DUAL;
-- EMP 테이블에서 사번, 사원명, 급여 조회 (단, 급여는 100단위 까지의 값만 출력 처리하고 급여 기준 내림차순 정렬)
SELECT EMPNO, ENAME, ROUND(SAL, -2)
FROM EMP
ORDER BY SAL DESC;
-- EMP 테이블에서 사원번호가 홀수인 사람들을 조회
SELECT *
FROM EMP
WHERE MOD(EMPNO, 2) != 0;
-- EMP 테이블에서 사원명, 입사일 조회 (단, 입사일은 년도와 월을 분리 추출해서 출력)
SELECT ENAME, 
    EXTRACT(YEAR FROM HIREDATE) AS 년도,
    EXTRACT(MONTH FROM HIREDATE) AS 월
FROM EMP;
-- EMP 테이블에서 9월에 입사한 직원의 정보 조회
SELECT *
FROM EMP
...
FROM EMP
WHERE ENAME LIKE '%E';
-- EMP 테이블에서 이름의 세번째 글자가 'R'인 직원의 정보 조회
SELECT *
FROM EMP
...
-- 자료형을 변환하는 형변환 함수
-- 자동 형변환 : NUMBER + 문자 타입 => NUMBER 타입으로 자동 변환
...
-- 주로 날짜 데이터를 문자데이터로 변환하는데 사용
-- 자바의 SimpleDateFormet()과 유사
...
SELECT SAL,
    ...
    TO_CHAR(SAL,'999,999,99') AS sal_4
FROM EMP;
...
-- NULL 처리 함수 : NULL운 값이 없음, 즉 할당되지 않음을 의미
...
-- NVL2() : NULL이 아닌경우와 NULL인 경우 모두에 대해서 값을 지정 가능하다.
